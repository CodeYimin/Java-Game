...
public class InputManager extends KeyAdapter {
  ...
  public void addKeyUpdateListener(
    Consumer<HashMap<Integer, Boolean>> listener
  ) {
  ...
  public <T> ArrayList<T> getActivatedValuesByBindName(
    String name,
    Class<T> type
  ) {
  ...
  public <T> void addCompositeBinding(String name, InputMap<T> inputMap) {
    if (activatedValuesByBindName.keySet().contains(name)) {
      throw new Error(
        "Composite binding with name " + name + " already exists."
      );
    }
