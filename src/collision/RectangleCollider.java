...
public class RectangleCollider implements Collider {
  ...
  public boolean collidesWith(
    ...
    if (otherCollider instanceof RectangleCollider) {
      ...
      double otherTopEdge =
        otherPosition.getY() + otherRect.getSize().getY() / 2;
      double otherBotEdge = otherTopEdge - otherRect.getSize().getY();
      double otherRightEdge =
        otherPosition.getX() + otherRect.getSize().getX() / 2;
      double otherLeftEdge = otherRightEdge - otherRect.getSize().getX();

      return (
        myTopEdge > otherBotEdge &&
        otherTopEdge > myBotEdge &&
        myRightEdge > otherLeftEdge &&
        otherRightEdge > myLeftEdge
      );
    } else {
      return false;
    }
